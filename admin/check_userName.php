...
if(!isset($_GET['userName'])){ // If they are trying to view this without ?username=username or ?password=password.
 ...
 // Requires config.php so we can access the database.
    ...
    $check=mysql_num_rows(mysql_query(" SELECT Username from ".$projectName."user  WHERE Username='".$user."'")); // Checks to see if a user is in the `users` table or not.
   
        if($check == 0){ // If there is no username in the database that matches the input one.
            echo '<i>'.$user.' is <span style="color:#009933">Username is Available!'; // Yay we can use it.
        }else{ // ElseIf there is a username in the database.
            echo '<i>'.$user.' is <span style="color:#CC0000">Not Available!</span>'.'</i>'; // None for you higgans.
        } // End ElseIf.
