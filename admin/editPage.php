<?php
	...
	if($_POST['editPage'] ){
		...
		if ($errormessage==''){
			$returnFromEdit=editPage_save_form($Pagetitle,$siteId,$content,$Seourl,$creationdate,$Rsspath,$formStackId,$fStackResponse,$mChimpListID,$mChimpResponse,$flagEbrite,$ccFlag,$ccResponse,$addStoreLocator,$haveUnqNavColor,$navColor,$isNav,$Pageorder,$parentLink,$publishContent);
		...
		if ($errormessage==''){
			$returnFromEdit=editPage_save_form($Pagetitle,$siteId,$content,$Seourl,$creationdate,$Rsspath,$formStackId,$mChimpListID,$flagEbrite,$ccFlag,$addStoreLocator,$isNav,$Pageorder,$parentLink,$publishContent);
			if ($returnFromEdit=="true"){
				$successmessage="Edit Successfully";
				if ($_POST['editPage']) {	
					$Pagetitle='';
...
<script language="javascript" type="text/javascript">
	...
	/*
	function displayResponseOption(element, displayThisElement){
		var divElement = document.getElementById(element).value;
		var displayElment = document.getElementById(displayThisElement);
                             ...
                             <tr id="formStack">
                                <td height="30"><h3>Add Formstack Form</h3>
                                    <div class="fieldExtraStyle">
                                        ...
                                        <a href="#dialog" name="modal" style="color: #F5C36D;">View Available Formstack Forms</a> <br />
                                        
                                        <? 
											//check to see if fsCheckResponse is empty
											if($row['fStackResponse'] == "" && $row['fStackFormId'] == 0) {
												$fsrIsChecked ="";
												$hideFSDiv = " style='display: none; background: #ccc;'";
											} else {
												$fsrIsChecked ="checked='checked'";
												$hideFSDiv = "";
											}
										?>
   <input type="checkbox" id="fsCheckResponse" name="fsCheckResponse" checked="checked" onclick="toggle_visibility('fsResponsex');" <?= $fsrIsChecked; ?> /> Add Custom Response after form submission
                                    </div>
...
<div id="fsResponsex" <?= $hideFSDiv; ?>>

<textarea name="fStackResponse"><? echo $row['fStackResponse']; ?> </textarea>
                                <script type="text/javascript">
                ...
                {
                    var editor = CKEDITOR.replace( 'fStackResponse' ,{
                                <?  $sql="SELECT usertype FROM ".$projectName."user WHERE Userid=$Userid";
                                $result=mysql_query($sql); 
                    ...
                    // CKFinder.SetupCKEditor( editor, { BasePath : '../../', RememberLastFolder : false } ) ;
                             ...
                             <tr id="mailchimp">
                                <td height="30"><h3>Add Mailchimp List ID</h3>
                                    ...
                                    <? 
											//check to see if fsCheckResponse is empty
											if($row['mChimpResponse'] == "") {
												$mcrIsChecked ="";
											} else {
												$mcrIsChecked ="checked='checked'";
											}
										?>
   <input type="checkbox" id="mcCheckResponse" name="mcCheckResponse" onclick="toggle_visibility('mcResponsex');" <?= $mcrIsChecked; ?> /> Add Custom Response after sign up
                                    </div>
...
<div id="mcResponsex"  style="display: none; background: #ccc;">

<textarea name="mChimpResponse"><? echo $row['mChimpResponse']; ?> </textarea>
                                <script type="text/javascript">
                ...
                {
                    var editor = CKEDITOR.replace( 'mChimpResponse' ,{
                                <?  $sql="SELECT usertype FROM ".$projectName."user WHERE Userid=$Userid";
                                $result=mysql_query($sql); 
                                    ...
                                    <div class="fieldExtraStyle" style="height: 20px;">
                                        ...
                                        <? 
											...
											if($row['ccResponse'] == "") {
												...
												$ccrIsChecked ="checked='checked'";
											}
										?>
   <input type="checkbox" id="ccCheckResponse" name="ccCheckResponse" onclick="toggle_visibility('ccResponsex');" <?= $ccrIsChecked; ?> /> Add Custom Response after form submission
                                    </div>
...
<div id="ccResponsex"  style="display: none; background: #ccc;">

<textarea name="ccResponse"><? echo $row['ccResponse']; ?> </textarea>
                                <script type="text/javascript">
                ...
                {
                    var editor = CKEDITOR.replace( 'ccResponse' ,{
                                <?  $sql="SELECT usertype FROM ".$projectName."user WHERE Userid=$Userid";
                                $result=mysql_query($sql); 
        ...
        filebrowserFlashUploadUrl : '/ckeditor/ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Flash' 
                    ...
                    if ($row['unqNavColor'] != ""){
                        $uncSelected = "checked='checked'";
                    } else {
						$uncSelected = "";	
                    }
                    ?>
                <td><strong>Add Unique Nav Color</strong> <br /><small><i>example: #cccccc for light gray</i></small><br /> 
                <input type="checkbox" id="haveUnqNavColor" name="haveUnqNavColor" value="1" <?= $uncSelected; ?> />Include Nav Color<br /><input class="fieldStyle color {hash:true,caps:false}" type="text" size="9" name="navColor" ID="navColor" value="<? echo $row['unqNavColor']; ?>"></td>	 
         	</tr> 
                            </table>
                     </div>
                 </div>
