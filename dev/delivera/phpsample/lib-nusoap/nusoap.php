...
class nusoap_base {
	...
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded'){
		...
		switch(true) {
			...
			case (is_string($val) || $type == 'string'):
				$val = $this->expandEntities($val);
				if ($use == 'literal') {
					$xml .= "<".rtrim("$name$xmlns $atts").">$val</$name>";
				} else {
    ...
    */
	...
	if($headers){
		if (is_array($headers)) {
			$xml = '';
			foreach ($headers as $hdrName => $header) {
				$xml .= $this->serialize_val($header, $hdrName, false, false, false, false, $use);
			}
			$headers = "<Header xmlns=\"http://delivra.com/\">".$xml."</Header>";
