...
<!--link rel="stylesheet" href="scripts/jquery.mobile.css" /> 
		<script type="text/javascript" src="scripts/js/jquery-1.7.1/jquery.min.js"></script>
		<script type="text/javascript" src="scripts/js/jquery-mobile-1.0/jquery.mobile.min.js"></script-->
		<script type="text/javascript" src="scripts/js/jquery.ui-1.8.15/jquery.ui.autocomplete.min.js"></script>
		<script type="text/javascript" src="scripts/ui/jquery.ui.map.js"></script>
		<script type="text/javascript" src="scripts/ui/jquery.ui.map.services.js"></script>
		<script type="text/javascript" src="scripts/ui/jquery.ui.map.extensions.js"></script>
...
<div id="styleTop">
    <p id="logo"><?php if($logoOrcompany['logo']!='')     { 
           ...
           ?> <img  src="/<?=$src?>" alt="Logo" height="<?PHP echo $logoOrcompany['logoHeight'] ?>" width="<?PHP echo $logoOrcompany['logoWidth'] ?>" />
                               <?php } else if($rowc['customSiteTitle'] != '') { echo "<h1 align='center'>".$rowc['customSiteTitle']."</h1>"; }?>
    </p>
    </div>
<?PHP 
if ($socialShare == '1') {
...
<? } ?>
	...
	?>
                <a href="#" onClick="history.go(-1);return false;" style="color: <?= $navStyle['searchBoxBG']; ?>;" target="_self">Back</a> | <a href="/" style="color: <?= $navStyle['searchBoxBG']; ?>;" target="_self">Home</a> | <? if (1==1){ ?> <a style="color: <?= $navStyle['searchBoxBG']; ?>;" href="tel:<?PHP echo $logoOrcompany['ocPhone']; ?>" class="nav" target="_self"  onclick="javascript:pageTracker._trackPageview ('clicktocall/tracking');"target="blank">Call</a> |<? } ?> <a style="color: <?= $navStyle['searchBoxBG']; ?>;" href="mailto:<?PHP echo $logoOrcompany['ocEmail']; ?>" class="nav" target="_self">Email</a>			
    <?PHP    
        } else {
	?>
...
if ($Pagetitle=="" && $searchSubmit =="" ){
	if($logoOrcompany['homepageText'] != ""){
		...
		echo "<div class='mobileContentArea1'><div >";
		if($userName == 'squishdesigns') {
			include("html5/index.html");
			$showHomePage .= " <br />";	
		}
		echo str_replace("/user_files/","/admin/user_files/", $logoOrcompany['homepageText']) . "</div></div>";
		//echo $showHomePage;
	}
	if($navigation == 1) {
		while( $row=mysql_fetch_array($result))  {
			if ($pageid != $row['Parentlink'] && $home == "yes" && $row['isNav'] != 0) {
			if($navStyle['markerColor'] != ""){
				$setNavMarker = '<div class="rightMarker">&nbsp;</div>';	
			} else {
				$setNavMarker = "";		
			}
			if($row['unqNavColor'] != ""){
				$showUnqNavColor = "style='background-color: " . $row['unqNavColor'] . ";'";
			}
				echo '<div class="m3link" '. $showUnqNavColor . '><a class="nav" accesskey="'.$ii.'" href="'.$row['Seourl'].'">'.$row['Pagetitle'].'</a>'. $setNavMarker. '</div>';
				print("\n");
				$showUnqNavColor ="";
				$ii++;
			}
	...
	//$rowy=mysql_fetch_array($resulty);
 	while( $rowy=mysql_fetch_array($resulty))  {
		if ($pageid == $rowy['Parentlink'] && $Pagetitle!="" && $rowy['isNav'] != 0) {
			...
			echo '<div class="m3link"><a class="nav" accesskey="'.$ii.'" href="'.$rowy['Seourl'].'">'.$rowy['Pagetitle'].'</a>' . $setNavMarker . '</div>';
			print("\n");
		}
		$ii++;
	}
...
<?PHP 
	...
	?>
<div  class="m3link "><a class="nav" href="<?= $logoOrcompany['mainWebsite'] . $indexCode; ?>">Full Site</a><div class="rightMarker">&nbsp;</div></div>
