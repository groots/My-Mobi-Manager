...
<?PHP if($navStyle['navStyle'] == 2) { ?>
	<?PHP if ($logoOrcompany['mainWebsite'] != "" || $logoOrcompany['mainWebsite'] != NULL) { 
    	i?>
...
<? if($fbId !=""){ ?>
    <strong class="fbShareTop">
    <a type="button" href="http://www.facebook.com/<?= $fbId; ?>"><img src="/images/icons/fbSmIcon.gif"  alt="Connet on Facebook" width="24" /></a>
    </strong>
<? } ?>    
<? if($twitterId !=""){ ?>
...
<div id="styleTop">
    <?php if($logoOrcompany['logo']!='')     { 
           ...
           ?> <a href="/"><img border="0" height="<?PHP echo $logoOrcompany['logoHeight'] ?>" width="<?PHP echo $logoOrcompany['logoWidth'] ?>" src="/<?=$src?>" alt="Logo" style="max-width: 230;" id="logo" /></a>
...
if ($Pagetitle=="" && $searchSubmit =="" ){
	if($logoOrcompany['homepageText'] != ""){ ?>
		...
		echo  str_replace("/user_files/","/admin/user_files/", $logoOrcompany['homepageText']); ?>
